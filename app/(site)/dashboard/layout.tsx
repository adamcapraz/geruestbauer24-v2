...
import { LayoutDashboard, MessageSquare, Settings, Building2, Plus, LogOut, ChevronLeft, Menu } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Skeleton } from "@/components/ui/skeleton"

const navItems = [
  { href: "/dashboard", label: "Übersicht", icon: LayoutDashboard },
  { href: "/dashboard/firma", label: "Meine Firma", icon: Building2 },
  { href: "/dashboard/anfragen", label: "Anfragen", icon: MessageSquare },
    ...
    <div className="flex min-h-[calc(100vh-4rem)]">
      ...
      `}>
        ...
        <nav className="flex-1 p-3 space-y-1">
          ...
          {/* Check if user has a firma, if not show "Firma eintragen" */}
          <Link
            href="/dashboard/firma/erstellen"
            onClick={() => setSidebarOpen(false)}
            className={`
              flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors
              ${pathname === "/dashboard/firma/erstellen"
                ? "bg-primary text-primary-foreground font-medium"
                : "text-slate-300 hover:bg-slate-800 hover:text-white"
              }
            `}

...
import { MessageSquare, Search, Clock, CheckCircle, Mail, Phone, Calendar, Filter, Eye } from "lucide-react"
...
export default function AnfragenPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "neu":
        return <Badge className="bg-blue-500 text-white hover:bg-blue-600"><Clock className="h-3 w-3 mr-1" />Neu</Badge>
      case "gelesen":
        return <Badge variant="secondary"><Eye className="h-3 w-3 mr-1" />Gelesen</Badge>
      case "beantwortet":
        return <Badge className="bg-green-600 text-white hover:bg-green-700"><CheckCircle className="h-3 w-3 mr-1" />Beantwortet</Badge>
      default:
          ...
          <CardContent className="pt-6 text-center py-12">
            ...
            <p className="text-muted-foreground">
              {anfragen.length === 0
                ...
                : "Keine Anfragen entsprechen Ihrem Filter."
              }
            </p>
          </CardContent>
        </Card>
          ...
          {filteredAnfragen.map((anfrage) => (
            <Card
              ...
              className={`cursor-pointer transition-colors hover:border-primary/50 ${
                anfrage.status === "neu" ? "border-l-4 border-l-blue-500" : ""
              }`}
      ...
      <Dialog open={!!selectedAnfrage} onOpenChange={(open) => !open && setSelectedAnfrage(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Eingegangen am{" "}
              {selectedAnfrage && new Date(selectedAnfrage.created_at).toLocaleDateString("de-DE", {
                day: "2-digit",
                month: "long",
                year: "numeric",
                hour: "2-digit",
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 p-3 bg-muted rounded-lg">
                <div>
                  ...
                  <a href={`mailto:${selectedAnfrage.email}`} className="text-sm font-medium text-primary hover:underline">
                    {selectedAnfrage.email}
                  </a>
                </div>
                {selectedAnfrage.telefon && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Telefon</p>
                    <a href={`tel:${selectedAnfrage.telefon}`} className="text-sm font-medium text-primary hover:underline">
                      {selectedAnfrage.telefon}
                    </a>
                  </div>
                )}
              ...
              {selectedAnfrage.status !== "beantwortet" && (
                <div className="space-y-2 border-t pt-4">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button
                      onClick={sendAntwort}
                      disabled={!antwortText.trim() || sending}
                    >
